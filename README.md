...
- **The Brain (§8–9)** — full map rebuild once per day. The 5-minute cron builds it at the 5am user-local rollover (scheduled precompute, §9.1), so the first open of the day shows the calculating screen for two seconds as an acknowledgement rather than a wait; if the cron missed it, first-open still rebuilds on demand behind the same screen. Items go in fresh; yesterday's bubbles are passed separately as "reuse only if apt" (anti-stickiness, §8.2); situation-name vocabulary carries forward for linguistic stability. Top-tier LLM call clusters by *situation*, ranks prominence qualitatively (urgency, importance, effort/runway, forgettability), rotates life-triggered KNOWs quietly, and event-linked KNOWs ride their situations. Same-day captures land in a deterministic **Captured Today** bucket; completions update the map in place with no Brain call.
...
Reader UI for the activity feed (§7.4), bubble pin/dismiss (§8.5), mid-day debounced rebuilds (§9.1), behavioural telemetry & learned notification timing (§11.6), numeric prominence weights (§9.2), Google Calendar sync (§13).
