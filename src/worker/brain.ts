...
MAP SHAPE: a good day is 4-7 cards; ten mostly-single-item cards is a list wearing bubbles, and the map's value is gone. Reach the shape by bundling first (the DOING bond exists precisely to fuse loose singles into one card) and by dropping second — dropping is real curation: quiet undated items don't need to appear every day; seen= shows what's had recent airtime, so rotate them through the days instead of listing them all. Two break-it-down invitations the same day compete with each other — surface the ripest one; the other returns another day. The map is the day's shape, not the inventory; browse holds everything.
