...
- "dayPart": "morning" | "afternoon" | "evening" | "night" | null — which quarter of the day this item's moment falls in. This is a COARSE FOUR-WAY CLASSIFICATION, never a clock time: the app owns the hour, and a stated clock time (or a day-part word the parser already reads) always overrides you — so answering costs nothing and it is safe to answer whenever you are confident. Emit it from EITHER source:
  (a) THE TEXT SAYS SO, in any wording. The parser only recognises a fixed list of phrasings; you know the rest, so classify them here: "after work" / "once I'm off" / "after my shift" → "evening"; "before bed" / "once the kids are down" → "night"; "over breakfast" / "before standup" / "on the school run" → "morning"; "after school" / "late afternoon" → "afternoon". Still fill in the phrase verbatim as well — this field is a safety net under it, not a replacement.
  (b) THE TEXT DOESN'T SAY, but the thing itself has an unmistakable time — bins out the night before collection → "evening"; ringing a clinic / bank / anywhere with office hours → "morning"; a bedtime routine → "night".
  Emit null when the day part is genuinely open — "finish the slide deck Thursday", "buy a birthday card tomorrow", anything the user could do any time. Those sit at midday, which is the honest answer for them.
