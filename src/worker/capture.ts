...
export async function handleCapture(env: Env, req: CaptureRequest): Promise<CaptureResponse> {
  ...
  for (const p of parsed.items) {
    ...
    // Deterministic date resolution (§12): the model only extracted phrases and
    // a day window. refineWithSourceTime recovers a clock time the extraction
    // dropped; withDayPart then places anything still day-only in its window.
...
- "dayPart": "morning" | "midday" | "afternoon" | "evening" | "night" | null — which window of the day this item's moment falls in. You pick the window; the app owns the hour, so never name a time here. Set it whenever the capture makes the window clear, from either source: the TEXT says so, in any wording at all ("tomorrow evening", "Wednesday morning", "tonight", "after work", "before bed", "first thing", "once the kids are down", "end of day"); or the text doesn't say but the THING has an unmistakable window (bins go out the evening before collection, ringing a clinic or a bank happens in the morning). A stated clock time is not this field — that belongs in the phrase, and it overrides this. Emit null when the window is genuinely open ("finish the slide deck Thursday", "buy a birthday card tomorrow"): those sit at midday, which is the honest answer for them.
